...
"select projects.title, SUM(pledges.amount) from projects LEFT JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title ORDER BY projects.title ASC;
"
end
